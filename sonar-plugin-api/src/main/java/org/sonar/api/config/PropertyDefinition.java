...
public final class PropertyDefinition {
  ...
  private static EnumMap<PropertyType, Validation> createValidations(List<String> options) {
    EnumMap<PropertyType, Validation> validations = new EnumMap<>(PropertyType.class);
    validations.put(BOOLEAN, validateBoolean());
    validations.put(INTEGER, validateInteger());
    validations.put(LONG, validateInteger());
    validations.put(FLOAT, validateFloat());
    validations.put(REGULAR_EXPRESSION, validateRegexp());
    validations.put(SINGLE_SELECT_LIST, aValue -> options.contains(aValue) ? Result.SUCCESS : Result.newError("notInOptions"));
    return validations;
  }
