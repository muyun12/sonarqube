...
public class SetActionTest {
  ...
  public void fail_when_data_and_login_type_with_invalid_login() {
    ...
    expectedException.expect(BadRequestException.class);
    expectedException.expectMessage("Error when validating login setting with key 'my.key' and values [login.1, unknown.login]. A value is not a valid login.");

    callForMultiValueGlobalSetting("my.key", newArrayList("login.1", "unknown.login"));
  }
